<div class="p-3 space-y-3 max-w-[100vw] mx-auto">
    ...
    <div class="grid grid-cols-1 gap-3">
        ...
        <div class="bg-white rounded-xl p-4 border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-all duration-200">
            <div>
            ...
            <div class="pt-3 border-t border-gray-100 flex items-center justify-between">
                ...
                @if(in_array(Auth::user()->role, ['Super Admin', 'RT']))
                <button onclick="hapusKegiatan({{ $item->id }})" class="w-7 h-7 rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-all inline-flex items-center justify-center" title="Hapus agenda">
        ...
        <div class="col-span-full bg-white p-8 rounded-xl border border-gray-100 shadow-sm text-center">
            <div class="flex flex-col items-center gap-3">
                ...
                <div>
                    <p class="font-bold text-gray-400 text-sm">Belum ada agenda kegiatan RT</p>
...
<div id="modal-tambah-kegiatan" class="hidden fixed inset-0 bg-black/60 z-50 flex items-end justify-center backdrop-blur-sm p-0">
    <div class="bg-white rounded-t-2xl w-full max-w-[95vw] p-5 relative shadow-2xl border border-gray-100 max-h-[90vh] overflow-y-auto">
        ...
        <h3 class="text-base font-black text-gray-800 mb-4">Tambah Agenda Kegiatan RT</h3>
        <form id="form-kegiatan" action="/kegiatan/store" method="POST" onsubmit="simpanDataUmum(event, 'form-kegiatan', 'kegiatan')">
            ...
            <div class="mt-5 flex justify-end gap-2">
                <button type="button" onclick="document.getElementById('modal-tambah-kegiatan').classList.add('hidden')" class="px-4 py-2 rounded-xl font-bold text-gray-500 hover:bg-gray-100 text-sm">Batal</button>
                <button type="submit" class="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl shadow-lg shadow-teal-200 text-sm">Simpan Agenda</button>
            </div>
        </form>
    </div>
</div>
